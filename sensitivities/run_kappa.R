...
for(model in c('bts', 'ats', 'combined')){
  for(kappascale in c(.5,1,2)){
    control <- list(n_x=100, n_eps1=0, n_eps2=0, n_omega2=1, n_omega1=1,
                    model=model, kappascale=kappascale)
    if(model=='combined')
      control[c('n_eps1', 'n_eps2', 'n_omega1', 'n_omega2')][3:4] <- "IID"
...
results.list <- lapply(list.files('sensitivities', full.names=TRUE,
...
g1 <- out %>% filter(model !='combined') %>%
  ggplot(aes(year, est, color=kappascale, group=kappascale)) +
  geom_line()+ facet_wrap('model', ncol=1) + ylab('log index')+ theme_bw()
...
g2 <- out %>% filter(model =='combined') %>%
  ggplot(aes(year, est, color=kappascale, group=kappascale)) +
  geom_line()+ facet_wrap('stratum', ncol=1) + ylab('log index')+theme_bw()
